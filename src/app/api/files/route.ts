...
import { FileMetadata } from "./fileStore";
  ...
  try {
    for (const chunkText of chunks) {
      ...
      if (embeddingVector.length > 0) {
        ...
        const chunkInsertResult: Database.RunResult = insertChunkStmt.run(
          docId,
          chunkText
        );
        // const chunkRowId = chunkInsertResult.lastInsertRowid;
        const chunkRowId: number | undefined =
          "id" in chunkInsertResult && typeof chunkInsertResult.id === "number"
            ? chunkInsertResult.id
            : "lastInsertRowid" in chunkInsertResult &&
                typeof chunkInsertResult.lastInsertRowid === "number"
              ? chunkInsertResult.lastInsertRowid
              : undefined; // 버전 호환성
...
export async function POST(req: NextRequest) {
  ...
  try {
    // 1. 요청에서 파일 정보 추출
    const { _file, fileBuffer, fileName, fileType, fileSize } =
